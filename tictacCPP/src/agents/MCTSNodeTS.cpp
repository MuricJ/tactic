...
void MCTSNodeTS::ConnectChild(MCTSNode* new_node_ptr){
    ...
    this->child_data.push_back(TSData());
}
...
void MCTSNodeTS::UpdateMethod(BoardState result, int child_index) {
    ...
    if (result == BoardState::DRAWN){
        this->child_data[child_index].parameter_alpha += 0.5;
        this->child_data[child_index].parameter_beta += 0.5;

    } else if (this->state.MoveNumber() % 2 == 0) { // P0 is on turn
        if (result == BoardState::WON_0) this->child_data[child_index].parameter_alpha += 1.0;
        else this->child_data[child_index].parameter_beta += 1.0;

    } else { // P1 is on turn
        if (result == BoardState::WON_1)  this->child_data[child_index].parameter_alpha += 1.0;
        else this->child_data[child_index].parameter_beta += 1.0;
    }
}
...
std::pair<int, float> MCTSNodeTS::GetMoveEval(){
    int best_move = -1;
    float best_move_eval = std::numeric_limits<float>::lowest();
